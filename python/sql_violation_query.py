...
class SQLViolationAnalyzer:
    """
    Analyzes violations using SQL queries based on the Python JSON analysis methodology.
    ...
    def analyze_violation_consequences(self, predicate_name: str, contract_name: Optional[str] = None) -> Dict:
        """
        ...
        for contract_id, contract_name in contracts:
            print(f"🔍 Analyzing violations in contract: {contract_name}")
            
            # Step 3: Find solutions where predicate is FALSE (vector analysis)
            # This is the key SQL query that replicates the Python logic
            query = """
            WITH violation_solutions AS (
                -- Find all solutions in this contract
                SELECT DISTINCT s.solution_id
                FROM solutions s
                WHERE s.contract_id = ?
                -- Exclude solutions that contain the target predicate
                AND s.solution_id NOT IN (
                    SELECT DISTINCT s2.solution_id
                    FROM solutions s2
                    JOIN predicates pred ON s2.predicate_id = pred.id AND s2.contract_id = pred.contract_id
                    WHERE pred.predicate_name = ? AND s2.contract_id = ?
                )
            ),
            ...
            predicate_violation_counts AS (
                SELECT 
                    cp.id,
                    cp.predicate_name,
                    cp.full_expression,
                    cp.predicate_type,
                    COUNT(DISTINCT s.solution_id) as violation_count
                FROM contract_predicates cp
                LEFT JOIN solutions s ON cp.id = s.predicate_id AND s.contract_id = ?
                INNER JOIN violation_solutions vs ON s.solution_id = vs.solution_id
                GROUP BY cp.id, cp.predicate_name, cp.full_expression, cp.predicate_type
            ),
            -- Get total violation count for this contract
            total_violations AS (
                SELECT COUNT(*) as total_count FROM violation_solutions
            )
            SELECT 
                pvc.predicate_name,
                pvc.full_expression,
                ...
                tv.total_count,
                CASE 
                    WHEN pvc.violation_count = tv.total_count THEN 'always_present'
                    WHEN pvc.violation_count = 0 THEN 'always_absent'
                    ELSE 'sometimes_present'
                END as consequence_type
            FROM predicate_violation_counts pvc
            CROSS JOIN total_violations tv
            WHERE pvc.violation_count = tv.total_count OR pvc.violation_count = 0
            ORDER BY consequence_type, pvc.predicate_name
            """
            
            cursor.execute(query, [contract_id, predicate_name, contract_id, contract_id, predicate_name, contract_id])
            consequences = [dict(row) for row in cursor.fetchall()]
            all_consequences.extend(consequences)
            
            # Get violation count for this contract
            cursor.execute("""
                SELECT COUNT(DISTINCT s.solution_id) as violation_count
                FROM solutions s
                WHERE s.contract_id = ?
                AND s.solution_id NOT IN (
                    SELECT DISTINCT s2.solution_id
            ...
            """, [contract_id, predicate_name, contract_id])
            
            violation_count = cursor.fetchone()['violation_count']
            total_violation_scenarios += violation_count
            
            print(f"   📊 Found {violation_count} violation scenarios")
            print(f"   📋 Found {len(consequences)} consequences")
        
        return {
            'predicate': predicate_name,
            'total_violation_scenarios': total_violation_scenarios,
...
def main():
    ...
    # Initialize analyzer
    analyzer = SQLViolationAnalyzer('enhanced_laml_contracts.db')
    
    print(f"\n📋 Analyzing violations for '{predicate_name}':")
    ...
    if violation['num_consequences'] > 0:
        ...
        if always_present:
            ...
            for cons in always_present[:5]:  # Show first 5
                print(f"   • {cons['predicate_name']} ({cons['predicate_type']}): {cons['violation_count']}/{cons['total_count']} scenarios")
